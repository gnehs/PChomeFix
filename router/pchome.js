...
router.get("/", async (ctx) => {
  ...
  <body class="markdown-body">
    ...
    <h3>透過 iOS 捷徑使用</h3>
    <p>輕觸「<a href="https://www.icloud.com/shortcuts/f2a73d3f47fe4832883d271230d0d84a" target="_blank">這裡</a>」來安裝 iOS 捷徑，透過分享功能快速複製連結。</p>
    <h2>相關連結</h2>
    <ul>
...
router.get(["/prod/:id", "/prod/:version/:id"], async (ctx) => {
  ...
  let img = `https://p.pancake.tw/og/pchome/${id}`;
...
router.get("/og/pchome/:id", async (ctx) => {
  const { id } = ctx.params;
  const bgColorList = [
    "#F8FAFC",
    "#F9FAFB",
